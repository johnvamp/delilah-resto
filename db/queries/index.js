...
function ordersTableQuery() {
	return `CREATE TABLE IF NOT EXISTS ${dbName}.orders (
        ...
        CONSTRAINT id_usuario FOREIGN KEY (id_usuario) REFERENCES users (id_usuario)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci`;
}
...
function selectQuery(table, columns = "*", conditions = null) {
	const query = `SELECT ${columns} FROM ${dbName}.${table}` + ` ${conditions ? `WHERE ${conditions}` : ""}`;
...
function updateQuery(table, changes, conditions) {
	const query = `UPDATE ${dbName}.${table} SET ${changes}` + `WHERE ${conditions}`;
	return query;
}
...
function joinQuery(mainTable, columns, joiners, conditions) {
	const fullJoiners = joiners
		...
		.replace(/,/g, "");
	const query =
		`SELECT ${columns} FROM ${dbName}.${mainTable}` + ` ${fullJoiners}` + `${conditions ? `WHERE ${conditions}` : ""}`;
	return query;
}
