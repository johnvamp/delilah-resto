...
//DATASETS
const usersDs = "../datasets/usuarios/users.csv";

const usersData = async () => {
	const parseStream = csv({ delimiter: "," });
	const data = await getStream.array(fs.createReadStream(usersDs).pipe(parseStream));
	return data;
};
...
const usersUpload = async () => {
	...
	for (let i = 0; i < dataToUpload.length; i++) {
		try {
			const { username, password, firstname, lastname, address, email, phone_number, is_admin } = dataToUpload[i];
